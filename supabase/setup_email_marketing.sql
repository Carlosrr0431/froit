...
DROP TABLE IF EXISTS froit_email_contacts CASCADE;

CREATE TABLE froit_email_contacts (
...
-- Índices para froit_email_contacts
CREATE INDEX IF NOT EXISTS idx_froit_email_contacts_email ON froit_email_contacts(email);
CREATE INDEX IF NOT EXISTS idx_froit_email_contacts_estado ON froit_email_contacts(estado);
CREATE INDEX IF NOT EXISTS idx_froit_email_contacts_origen ON froit_email_contacts(origen);
CREATE INDEX IF NOT EXISTS idx_froit_email_contacts_created_at ON froit_email_contacts(created_at DESC);

-- Comentarios
COMMENT ON TABLE froit_email_contacts IS 'Contactos para campañas de email marketing - Froit';
COMMENT ON COLUMN froit_email_contacts.estado IS 'Estado del contacto: activo, inactivo, bounce, spam, unsubscribed';
COMMENT ON COLUMN froit_email_contacts.origen IS 'Origen del contacto: excel_import, manual, api, landing';


-- 2. TABLA: email_campaigns
-- Almacena las campañas de email marketing
DROP TABLE IF EXISTS froit_email_campaigns CASCADE;

CREATE TABLE froit_email_campaigns (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    ...
    -- Remarketing
    campaña_padre_id UUID REFERENCES froit_email_campaigns(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_froit_email_campaigns_estado ON froit_email_campaigns(estado);
CREATE INDEX IF NOT EXISTS idx_froit_email_campaigns_tipo ON froit_email_campaigns(tipo);
CREATE INDEX IF NOT EXISTS idx_froit_email_campaigns_created_at ON froit_email_campaigns(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_froit_email_campaigns_programada_para ON froit_email_campaigns(programada_para);
CREATE INDEX IF NOT EXISTS idx_froit_email_campaigns_padre ON froit_email_campaigns(campaña_padre_id);

-- Comentarios
COMMENT ON TABLE froit_email_campaigns IS 'Campañas de email marketing de Froit';
COMMENT ON COLUMN froit_email_campaigns.estado IS 'Estado: borrador, enviando, completada, pausada, cancelada, programada';
COMMENT ON COLUMN froit_email_campaigns.tipo_remarketing IS 'Tipo: no-abierto, sin-click, abandonados';


-- 3. TABLA: email_sends
-- Registro individual de cada email enviado
DROP TABLE IF EXISTS froit_email_sends CASCADE;

CREATE TABLE froit_email_sends (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    campaign_id UUID REFERENCES froit_email_campaigns(id) ON DELETE CASCADE NOT NULL,
    contact_id UUID REFERENCES froit_email_contacts(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_froit_email_sends_campaign ON froit_email_sends(campaign_id);
CREATE INDEX IF NOT EXISTS idx_froit_email_sends_contact ON froit_email_sends(contact_id);
CREATE INDEX IF NOT EXISTS idx_froit_email_sends_email ON froit_email_sends(email);
CREATE INDEX IF NOT EXISTS idx_froit_email_sends_estado ON froit_email_sends(estado);
CREATE INDEX IF NOT EXISTS idx_froit_email_sends_abierto ON froit_email_sends(abierto);
CREATE INDEX IF NOT EXISTS idx_froit_email_sends_click ON froit_email_sends(click);
CREATE INDEX IF NOT EXISTS idx_froit_email_sends_bounce ON froit_email_sends(bounce);
CREATE INDEX IF NOT EXISTS idx_froit_email_sends_brevo_message_id ON froit_email_sends(brevo_message_id);
CREATE INDEX IF NOT EXISTS idx_froit_email_sends_fecha_envio ON froit_email_sends(fecha_envio DESC);

-- Comentarios
COMMENT ON TABLE froit_email_sends IS 'Registro individual de emails enviados con tracking completo';
COMMENT ON COLUMN froit_email_sends.bounce_type IS 'Tipo de bounce: hard (permanente), soft (temporal), blocked';


-- 4. TABLA: email_daily_counter
-- Contador diario de emails enviados (límite Brevo)
DROP TABLE IF EXISTS froit_email_daily_counter CASCADE;

CREATE TABLE froit_email_daily_counter (
...
-- Índice
CREATE INDEX IF NOT EXISTS idx_froit_email_daily_counter_fecha ON froit_email_daily_counter(fecha DESC);

-- Comentarios
COMMENT ON TABLE froit_email_daily_counter IS 'Contador diario de emails para respetar límite de Brevo (300/día)';


-- 5. TABLA: email_templates
-- Plantillas HTML reutilizables
DROP TABLE IF EXISTS froit_email_templates CASCADE;

CREATE TABLE froit_email_templates (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    nombre TEXT NOT NULL,
    descripcion TEXT,
...
-- Índices
CREATE INDEX IF NOT EXISTS idx_froit_email_templates_tipo ON froit_email_templates(tipo);
CREATE INDEX IF NOT EXISTS idx_froit_email_templates_activo ON froit_email_templates(activo);

-- Comentarios
COMMENT ON TABLE froit_email_templates IS 'Plantillas HTML reutilizables para campañas';
...
CREATE TRIGGER update_email_contacts_updated_at BEFORE UPDATE ON froit_email_contacts
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_email_campaigns_updated_at BEFORE UPDATE ON froit_email_campaigns
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_email_sends_updated_at BEFORE UPDATE ON froit_email_sends
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_email_daily_counter_updated_at BEFORE UPDATE ON froit_email_daily_counter
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_email_templates_updated_at BEFORE UPDATE ON froit_email_templates
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
DROP VIEW IF EXISTS v_campaigns_summary;
...
INSERT INTO froit_email_templates (nombre, descripcion, tipo, html_contenido, texto_plano, es_predeterminada, variables)
VALUES (
...
ALTER TABLE froit_email_contacts ENABLE ROW LEVEL SECURITY;
ALTER TABLE froit_email_campaigns ENABLE ROW LEVEL SECURITY;
ALTER TABLE froit_email_sends ENABLE ROW LEVEL SECURITY;
ALTER TABLE froit_email_daily_counter ENABLE ROW LEVEL SECURITY;
ALTER TABLE froit_email_templates ENABLE ROW LEVEL SECURITY;

-- Políticas: Permitir todo para usuarios autenticados (ajustar según necesidad)
CREATE POLICY "Permitir todo a usuarios autenticados" ON froit_email_contacts FOR ALL USING (true);
CREATE POLICY "Permitir todo a usuarios autenticados" ON froit_email_campaigns FOR ALL USING (true);
CREATE POLICY "Permitir todo a usuarios autenticados" ON froit_email_sends FOR ALL USING (true);
CREATE POLICY "Permitir todo a usuarios autenticados" ON froit_email_daily_counter FOR ALL USING (true);
CREATE POLICY "Permitir todo a usuarios autenticados" ON froit_email_templates FOR ALL USING (true);
